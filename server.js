...
app.get('/proyecto-detalle/:id', requireLogin, requireAdminOrCoord, async (req, res) => {
    ...
    try {
        ...
        // 2. Traemos todos los datos (Pagos, Gastos, Suplidores, Ajustes)
        const [paymentsResult, expensesResult, suppliersResult, adjustmentsResult] = await Promise.all([
            client.query(`SELECT * FROM payments WHERE quote_id = $1 ORDER BY payment_date DESC`, [quote.id]),
            client.query(`SELECT e.*, s.name as supplier_name FROM expenses e JOIN suppliers s ON e.supplier_id = s.id WHERE e.quote_id = $1 ORDER BY e.expense_date DESC`, [quote.id]),
            client.query('SELECT * FROM suppliers ORDER BY name ASC'),
        ...
        // 4. Generación de HTML para Tablas
        let adjustmentsHtml = adjustmentsResult.rows.map(adj => `
            <tr><td>${new Date(adj.fecha_ajuste).toLocaleString('es-DO')}</td><td style="color: ${adj.monto_ajuste >= 0 ? 'green' : 'red'}; font-weight: bold;">${adj.monto_ajuste >= 0 ? '+' : ''}$${parseFloat(adj.monto_ajuste).toFixed(2)}</td><td>${adj.motivo}</td><td>${adj.ajustado_por}</td></tr>
        `).join('') || '<tr><td colspan="4">No hay ajustes.</td></tr>';

        let paymentsHtml = payments.map(p => `
            <tr><td>${new Date(p.payment_date).toLocaleDateString()}</td><td>$${parseFloat(p.amount).toFixed(2)}</td><td>${p.students_covered || 'N/A'}</td><td>${p.comment || ''}</td><td style="text-align: center;"><a href="/recibo-pago/${p.id}/pdf" target="_blank" class="btn" style="background-color: #17a2b8;">Recibo</a></td></tr>
        `).join('') || '<tr><td colspan="5">No hay pagos.</td></tr>';

        let expensesHtml = expenses.map(e => `
            <tr><td>${new Date(e.expense_date).toLocaleDateString()}</td><td>${e.supplier_name}</td><td>${e.description}</td><td>$${parseFloat(e.amount).toFixed(2)}</td><td>${e.type || ''}</td><td style="text-align: center;"><a href="/desembolso/${e.id}/pdf" target="_blank" class="btn">Imprimir</a></td></tr>
        `).join('') || '<tr><td colspan="6">No hay gastos.</td></tr>';

        let suppliersOptionsHtml = suppliers.map(s => `<option value="${s.id}">${s.name}</option>`).join('');

        // 5. Envío del HTML Completo (Incluye Formularios y Scripts)
        res.send(`
            <!DOCTYPE html><html lang="es"><head>${commonHtmlHead}<style>.admin-notes { background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; }</style></head>
            <body>
                <div class="container">
                    ${backToDashboardLink}
                    <div class="header">
                    ...
                    ${quote.notas_administrativas ? `<div class="admin-notes"><strong>Notas:</strong><br>${quote.notas_administrativas}</div>` : ''}
                    
                    <div class="summary">
                        <div class="summary-box">
                            <div style="display:flex; justify-content:space-between;"><h3>Monto Venta</h3><button onclick="abrirModalAjuste()" class="btn" style="padding:2px 5px; font-size:10px;">Ajustar</button></div>
                            <p class="amount">$${totalVenta.toFixed(2)}</p>
                        </div>
                        <div class="summary-box"><h3>Total Abonado</h3><p class="amount green">$${totalAbonado.toFixed(2)}</p></div>
                        <div class="summary-box"><h3>Total Gastado</h3><p class="amount orange">$${totalGastado.toFixed(2)}</p></div>
                    ...
                    <button class="btn" onclick="toggleForm('payment-form')">Registrar Abono</button>
                    <div id="payment-form" class="form-container" style="display:none;">
                        <form action="/proyecto/${quote.id}/nuevo-pago" method="POST"><div class="form-group"><label>Fecha:</label><input type="date" name="payment_date" required></div><div class="form-group"><label>Monto:</label><input type="number" name="amount" step="0.01" required></div><div class="form-group"><label>Estudiantes:</label><input type="number" name="students_covered"></div><div class="form-group"><label>Comentario:</label><textarea name="comment"></textarea></div><button type="submit" class="btn">Guardar</button></form>
                    </div>
                    ...
                    <button class="btn btn-gasto" onclick="toggleForm('expense-form')">Registrar Gasto</button>
                    <div id="expense-form" class="form-container" style="display:none;">
                        <form action="/proyecto/${quote.id}/nuevo-gasto" method="POST"><div class="form-group"><label>Fecha:</label><input type="date" name="expense_date" required></div><div class="form-group"><label>Suplidor:</label><select name="supplier_id" required><option value="">Seleccione...</option>${suppliersOptionsHtml}</select></div><div class="form-group"><label>Monto:</label><input type="number" name="amount" step="0.01" required></div><div class="form-group"><label>Descripción:</label><textarea name="description" required></textarea></div><button type="submit" class="btn">Guardar</button></form>
                    </div>
                </div>
                <script>
                    function toggleForm(id) { const e = document.getElementById(id); e.style.display = e.style.display === 'none' ? 'block' : 'none'; }
                    async function abrirModalAjuste() {
                        const monto = prompt("Monto a ajustar (+/-):");
                        const motivo = prompt("Motivo:");
                        const codigo = prompt("Código de seguridad:");
                        if (monto && motivo && codigo) {
                            const res = await fetch('/proyecto/${quote.id}/ajustar-monto', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ monto_ajuste: parseFloat(monto), motivo, codigo_secreto: codigo }) });
                            if (res.ok) window.location.reload(); else alert("Error al ajustar");
                        }
                    }
                </script>
            </body></html>`);
        ...
        if (client) client.release(); // Seguridad de conexión
    }
});
